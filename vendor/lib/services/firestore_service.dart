...
class FirestoreService {
  ...
  /// Real-time stream of products created by this vendor.
  static Stream<List<Product>> watchVendorProducts({required String vendorId}) {
    return products
  ...
  /// One-time fetch of product IDs owned by this vendor.
  static Future<Set<String>> fetchVendorProductIds(
      {required String vendorId}) async {
    try {
      final snap =
          await products.where('vendorId', isEqualTo: vendorId).get();
  ...
  /// Delete a product document and decrement its category's productCount.
  static Future<void> deleteProduct({
    ...
    if (categoryId.isNotEmpty) {
      await _db.collection('categories').doc(categoryId).update({
        'productCount': firestore.FieldValue.increment(-1),
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // Vendor Orders — server-side filtered, no full collection scan
  // ─────────────────────────────────────────────────────────────────────────────

  /// Real-time stream of orders that contain this vendor's products.
  /// Uses the `vendorIds` array field written by checkout when order is created.
  static Stream<List<Order>> watchVendorOrders({required String vendorId}) {
    return orders
  ...
  /// Update a single order's status field. Propagates in real-time to
  /// Customer App and Admin Panel via their own Firestore listeners.
  ...
  /// Vendor accepts the order — marks vendorAccepted and sets status to 'accepted'.
  static Future<void> acceptOrder({required String orderId}) async {
    await orders.doc(orderId).update({
      'status': 'accepted',
      'vendorAccepted': true,
      'vendorAcceptedAt': firestore.FieldValue.serverTimestamp(),
      'updatedAt': firestore.FieldValue.serverTimestamp(),
    });
  }

  /// Vendor rejects the order — sets status to 'rejected'.
  static Future<void> rejectOrder({required String orderId}) async {
    await orders.doc(orderId).update({
      'status': 'rejected',
  ...
  /// Update product fields (used by edit product flow).
  ...
  /// Toggle product status between active and paused.
  ...
  /// Fetch a user document (for phone lookup in order details).
