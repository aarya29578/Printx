...
export default function OrdersPage() {
  ...
  const { orders, updateStatus, loadOrders } = useOrdersStore()
  ...
  console.log('ORDERS STATE COUNT', safeOrders.length)


  const tabs = [
    { label: `All (${safeOrders.length})`, value: 'all' },
    { label: `Pending (${safeOrders.filter((o) => o?.status === 'pending').length})`, value: 'pending' },
    { label: `Design Review (${safeOrders.filter((o) => o?.status === 'design_review').length})`, value: 'design_review' },
    { label: `Printing (${safeOrders.filter((o) => o?.status === 'printing').length})`, value: 'printing' },
    { label: `Shipped (${safeOrders.filter((o) => o?.status === 'shipped').length})`, value: 'shipped' },
    { label: `Delivered (${safeOrders.filter((o) => o?.status === 'delivered').length})`, value: 'delivered' },
    { label: `Cancelled (${safeOrders.filter((o) => o?.status === 'cancelled').length})`, value: 'cancelled' },
  ]


  const filtered = useMemo(
    () =>
      (safeOrders || []).filter((item) => {
        const q = (query || '').toLowerCase()
        const id = item?.id ?? ''
        const customerName = item?.customer?.name ?? ''
        const matchesQuery = id.toLowerCase().includes(q) || customerName.toLowerCase().includes(q)
        const matchesPayment = payment === 'all' || (item?.payment ?? null) === payment
        const matchesStatus = statusTab === 'all' || (item?.status ?? null) === statusTab
        return matchesQuery && matchesPayment && matchesStatus
      }),
    [safeOrders, query, payment, statusTab],
  )
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Orders"
        subtitle={`${safeOrders.length} total orders`}
